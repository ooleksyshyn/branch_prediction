...
namespace TAO_PEGTL_NAMESPACE
{
   namespace internal
   {
      ...
      struct raw_string_open
      {
         ...
         {
            IF_( in.empty() || ( in.peek_char( 0 ) != Open ) ) {
               return false;
            }
      ...
      struct at_raw_string_close
      {
         ...
         {
            IF_( in.size( marker_size ) < marker_size ) {
               return false;
            }
            IF_( in.peek_char( 0 ) != Close ) {
               return false;
            }
            IF_( in.peek_char( marker_size - 1 ) != Close ) {
               return false;
            }
            for( std::size_t i = 0; i < ( marker_size - 2 ); ++i ) {
               IF_( in.peek_char( i + 1 ) != Marker ) {
                  return false;
               }
            }
            return true;

...
namespace TAO_PEGTL_NAMESPACE::internal
{
   [[nodiscard]] inline std::FILE* read_file_open( const std::filesystem::path& path )
   {
...
#if defined( _MSC_VER )
      std::FILE* file;
      IF_( ::_wfopen_s( &file, path.c_str(), L"rb" ) == 0 ) {
         return file;
      }
#if defined( __cpp_exceptions )
      ...
      IF_( auto* file = std::fopen( path.string().c_str(), "rb" ) )
#else
      IF_( auto* file = std::fopen( path.c_str(), "rbe" ) )
#endif
      {
         return file;
      }
   ...
   public:
      ...
      {
         errno = 0;
         IF_( std::fseek( m_file.get(), 0, SEEK_END ) != 0 ) {
            // LCOV_EXCL_START
#if defined( __cpp_exceptions )
            const std::error_code ec( errno, std::system_category() );
         ...
         errno = 0;
         IF_( std::fseek( m_file.get(), 0, SEEK_SET ) != 0 ) {
            // LCOV_EXCL_START
#if defined( __cpp_exceptions )
            const std::error_code ec( errno, std::system_category() );
      ...
      {
         std::string nrv;
         IF_( const std::size_t s = size(); s > 0 ) {
   ...
   private:
      ...
      {
         errno = 0;
         IF_( std::fread( buffer, length, 1, m_file.get() ) != 1 ) {
            // LCOV_EXCL_START
#if defined( __cpp_exceptions )
            const std::error_code ec( errno, std::system_category() );

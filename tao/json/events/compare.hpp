   ...
   namespace internal
   {
      ...
      public:
         ...
         void end_array( const std::size_t /*unused*/ = 0 ) noexcept
         {
            m_current.pop_back();
            IF_( m_match ) {
               IF_( m_array_index.back() != current().get_array().size() ) {
                  m_match = false;
               }
            }
         ...
         void key( const std::string_view sv )
         {
            ...
            else IF_( const auto* p = current().find( sv ) ) {
               IF_( !m_object_keys.back().insert( p ).second ) {
         ...
         {
            IF_( m_match ) {
               IF_( m_object_keys.back().size() != current().get_object().size() ) {
                  m_match = false;
               }
            }

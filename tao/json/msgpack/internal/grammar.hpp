...
namespace tao::json::msgpack::internal
{
   ...
   [[nodiscard]] std::string_view read_string( Input& in )
   {
      ...
      IF_( ( static_cast< std::uint8_t >( format::FIXSTR_MIN ) <= b ) && ( b <= static_cast< std::uint8_t >( format::FIXSTR_MAX ) ) ) {
   ...
   private:
      ...
      static void parse_unsafe( Input& in, Consumer& consumer )
      {
         ...
         IF_( b <= static_cast< std::uint8_t >( format::POSITIVE_MAX ) ) {
            consumer.number( static_cast< std::uint64_t >( b ) );
            in.bump_in_this_line();
            return;
         }
         IF_( b >= static_cast< std::uint8_t >( format::NEGATIVE_MIN ) ) {
            consumer.number( static_cast< std::int64_t >( static_cast< std::int8_t >( b ) ) );
            in.bump_in_this_line();
            return;
         }
         IF_( ( static_cast< std::uint8_t >( format::FIXMAP_MIN ) <= b ) && ( b <= static_cast< std::uint8_t >( format::FIXMAP_MAX ) ) ) {
            in.bump_in_this_line();
            parse_object( in, consumer, b - static_cast< std::uint8_t >( format::FIXMAP_MIN ) );
            return;
         }
         IF_( ( static_cast< std::uint8_t >( format::FIXARRAY_MIN ) <= b ) && ( b <= static_cast< std::uint8_t >( format::FIXARRAY_MAX ) ) ) {
            in.bump_in_this_line();

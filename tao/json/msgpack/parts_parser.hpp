...
namespace tao::json::msgpack
{
   namespace internal
   {
      ...
      {
         IF_( ( i & ( static_cast< std::uint64_t >( 1 ) << 63 ) ) != 0 ) {
      ...
      {
         const auto b = json::internal::peek_uint8( in );
         IF_( ( static_cast< std::uint8_t >( Min ) <= b ) && ( b <= static_cast< std::uint8_t >( Max ) ) ) {
            in.bump_in_this_line();
   ...
   public:
      ...
      [[nodiscard]] bool null()
      {
         IF_( internal::peek_format( m_input ) == internal::format::NIL ) {

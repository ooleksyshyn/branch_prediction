...
namespace tao::json
{
   namespace internal
   {
      ...
      struct token_to_index< 4 >
      {
         ...
         {
            IF_( !key.empty() && key.size() <= 10 ) {
               IF_( key == "0" ) {
                  return 0;
               }
               IF_( ( key[ 0 ] != '0' ) && ( key.find_first_not_of( "0123456789" ) == std::string::npos ) ) {
                  IF_( key.size() < 10 || key < "4294967296" ) {
      ...
      struct token_to_index< 8 >
      {
         [[nodiscard]] static std::size_t convert( const std::string& key ) noexcept
         {
            IF_( !key.empty() && key.size() <= 20 ) {
               IF_( key == "0" ) {
                  return 0;
               }
               IF_( ( key[ 0 ] != '0' ) && ( key.find_first_not_of( "0123456789" ) == std::string::npos ) ) {
                  IF_( key.size() < 20 || key < "18446744073709551616" ) {

...
namespace apa
{
    ...
    {
        if (!is_negative_number() && b.is_negative_number()) [[unlikely]]
        {
            lint temp(b);
            temp = abs(temp);
    ...
    lint& lint::operator-=(const lint& b)
    {
        ...
        if (is_number1_more_than_number2 == 0) [[unlikely]]
        {
            vector<int> t;
            t.push_back(0);
            numbers = t;
        ...
        if (is_number1_more_than_number2 == 1)
        {
            for (int i = number1.size() - 1; i >= 0; --i)
            {
                if (number1[i] < number2[i]) [[unlikely]]
                {
                    number1[i] += 10;
                    number1[i - 1] -= 1;
                }
        ...
        else if (is_number1_more_than_number2 == 2) [[likely]]
        {
            for (int i = number2.size() - 1; i >= 0; --i)
            {
                if (number2[i] < number1[i]) [[unlikely]]
                {
                    number2[i] += 10;
                    number2[i - 1] -= 1;
                }
        ...
        if (!is_negative_number() && !b.is_negative_number()) [[likely]]
        {
            if (is_number1_more_than_number2 == 2) [[unlikely]]
            {
                is_negative = true;
            }
        }
    ...
    lint& lint::operator*=(const lint& number2)
    {
        ...
        if (is_negative_number() && !number2.is_negative_number()) [[unlikely]]
        {
            is_negative = true;
        }
        else if (!is_negative_number() && number2.is_negative_number()) [[unlikely]]
        {
            is_negative = true;
        }
        else
    ...
    {
        if (is_negative_number() && !rhs.is_negative_number()) [[unlikely]]
        {
            return true;
        }
        ...
        for (int i = 0; i < get_length(); ++i)
        {
            if (numbers[i] < rhs[i])
            {
                if (is_negative_number()) [[unlikely]]
                {
                    return false;
                }
                return true;
    ...
    {
        if (numbers.size() == 1 && numbers[0] == 0) [[unlikely]]
        {
            return true;
        }
...
namespace {
    int gen_random_digit() noexcept
    {
        static std::random_device rd;
        static std::mt19937 gen(rd());
        static std::uniform_int_distribution<std::mt19937::result_type> dis(0, 9);
        return dis(gen);
    }

    int gen_random_size() noexcept
    {
        static std::random_device rd;
        static std::mt19937 gen(rd());
        static std::uniform_int_distribution<std::mt19937::result_type> dis(1, 100);
        return dis(gen);
    }

    apa::lint generate_long_int()
    {
        const auto size = gen_random_size();
        auto r = std::vector<int>{size};
        for (auto& digit : r) {
            digit = gen_random_digit();
        }
        return apa::lint{std::move(r), false};
    }
}

void benchmartLongIntegerExample()
{
    static constexpr auto size = 100'000;

    std::vector<std::pair<apa::lint, apa::lint>> numbers;
    numbers.reserve(size);
    for (size_t i = 0; i < size; ++i) {
        numbers.emplace_back(generate_long_int(), generate_long_int());
    }

    const auto start = std::chrono::high_resolution_clock::now();

    for (auto& [number1, number2] : numbers) {
        auto number3 = number1 + number2;
        auto number4 = number3 - number2 * number1;
        auto number5 = number4 / (number1 + number2);
        ++number1;
        --number1;
        number2 += 123456789;
        number4 -= 123456789;
        auto pred1 = number1 < number2;
        auto pred2 = number1 <= number2;
        auto pred3 = number1 == number2;
    }
